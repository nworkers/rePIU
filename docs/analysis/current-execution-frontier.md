...
| 1 | **[해소, Tasks 438·439] draw batching으로 게이트 비중 10.35% → 8.40%** — 배치 평균 **16.02**(최대 332), 크로싱당 −23.7%, `failures`·`voided`·구현 공백 0, 시각 회귀 없음. 기본값으로 승격했고 `REPIU_GLIDE_DRAW_BATCH=0`이 대조군입니다. 남은 Glide 후보는 (b) `grBufferSwap` 드라이버 작업과 (c) `grTexSource`의 bind당 왕복 32.1/프레임 | ~~다음 Glide 축은 draw batching입니다~~ — 실부하 gameplay에서 게이트 크로싱의 **69.9%가 `grDrawTriangle`**이고(프레임당 670.8), `DrawTriangle`은 **삼각형당 host rendezvous 1회**입니다. setter 축은 Task 365/437이 이미 걷어냈고(생략 221.3/프레임, 적용 1.8), 437이 없앤 96회는 draw의 7분의 1입니다. 후보 (b) `grBufferSwap` 드라이버 작업과 (c) 남은 왕복은 그 다음 | 삼각형을 모아 한 번에 넘기면 rendezvous가 프레임당 670 → 약 100(비-draw 게이트 수)으로 떨어집니다 | gate가 guest-run의 **약 50%** |
...
### [Task 430 판정] 항목 1‴ 확정 — 게스트 시계가 실시간의 51%로 갑니다

**사용자가 점프 증상을 확인한 실행**(2026-08-06, pumpit1, 47초)의 census입니다.

```
timer tick delivery due/injected/coalesced/dropped/deferred: 10942/5661/5256/25/0
```

| 본곡 구간(gen 7, 28.34초) | 값 |
|---|---|
| 음악 | **74.97 LBA/s** — 정확(오차 0.04%) |
| `injected/due` | **약 51%** (기준 ≤ 96% → 확정) |
| `tick_lag_ms` | 8,810 → **22,666** (**+13,856 ms**, 기준 > 1,000 ms → 확정) |

**음악이 28.3초 흐르는 동안 게스트 시계는 14.5초만 흘렀습니다.**

**시계열이 아니면 안 보이는 구조:** 같은 실행에서 프리뷰 구간(gen 3·5·6)은 전달률
**약 100%** 이고 `tick_lag_ms`가 오히려 **감소**(9,006 → 8,793)하는데, 본곡 구간만
**51%** 입니다. 누적 51.7% 한 줄은 두 구간 중 어느 것도 설명하지 않습니다.

### [Task 432 해소] 노트·BGA 점프 축이 닫혔습니다

**사용자가 `REPIU_TIMER_TICK_BACKLOG=1`로 gameplay를 하고 점프 증상 해소를
확인했습니다**(2026-08-06). 같은 실행 본곡 구간 64.53초·표본 591개:
...
**Task 366의 프레임 −16.4%는 현재 빌드에서 재현되지 않습니다.** 그 비용의 기전은 366이
스스로 특정한 "safe point 상시 armed"였고, 그것은 backlog가 상한에 고착될 때만
...
### [Task 431 판정] 손실의 93.9%는 Glide 게이트 안에서 일어납니다

같은 사용자 실행 계열(2026-08-06, pumpit1)의 본곡 구간 22.77초, 표본 208개입니다.

| 지표 | 값 |
|---|---|
| 음악 | **74.98 LBA/s**(오차 0.03%) |
| 전달률 | **50.6%** |
| **`coalesced_in_gate / coalesced`** | **2,544 / 2,710 = 93.9%** (기준 ≥ 80%) |
| `tick_lag_ms` | 3,898 → 15,263 → 게스트 시계 = 실시간의 **50.1%** |
| 검산 `safe_point_traps / injected` | 2,765 / 2,781 = **0.99** |
| 검산 본곡 `safe_point_traps`/초 | **121.4**(240 아님 → 모순 없음) |

`InvokeOnHostThread`가 `host_command_cv_.wait()`로 게스트 스레드를 세우는 동안 게스트
코드가 한 줄도 실행되지 않으므로, 안전점(캐시 내 INT3 **967개**)은 그 창에서 **하나도
밟히지 않습니다.** 그동안 due는 240 Hz로 계속 쌓여 `bool` 하나에 합쳐집니다. **따라서
그 틱들은 늦은 것이 아니라 전달 불가능했습니다.**

**기전은 `deferred = 0`이 좁혀 줍니다.** 주입이 IF=0이나 비게스트 EIP로 막힌 적이
**한 번도 없습니다.** 즉 거부가 아니라 **도달 빈도**의 문제이며, 주입 5,661회/47초 =
**초당 약 120회**로 due 240 Hz의 절반입니다. 밀린 틱이 평균 8.3 ms 기다리다 절반이
coalesce됩니다. **따라서 backlog 옵션으로는 고쳐지지 않습니다** — 기회가 모자라면
상한 64에 붙고 초과분이 `dropped`가 되며, 그것이 Task 366의 T3입니다.

### [Task 430 이전 기록] 계측을 넣기까지

**새 사실 — 게스트 메인 루프는 실시간 60 Hz입니다.** Task 422 trace를 재실행 없이
재집계했습니다. gameplay 137초 중 이상치는 **양끝 잘린 버킷 2건뿐**이고, 정밀 구간은
**7,200 polls / 119.968 s = 60.0160 Hz(오차 0.03%)** 입니다. 루프가 4틱(240 Hz ÷ 4)
대기라면 4% 손실 시 57.6 Hz여야 하므로 **후보 1‴은 현재 데이터와 충돌**합니다.

**그러나 누적 한 줄로는 판정할 수 없습니다.** `coalesced 1,677 / due 41,531 = 4.04%`는
174초 **전체 평균**이라 부팅·attract에 몰렸는지 gameplay에 있는지 말하지 않습니다.
Task 366이 11.9% 손실을 잰 것도 attract 구간입니다.

**Task 430이 그 계측을 넣었습니다** — census 표본에 `ticks_due`·`ticks_injected` 델타와
파생 열 `tick_lag_ms`(누적 게스트 시계 지연). **동작 불변**이며 판정 규칙은
[설계 §6](../design/20260805-430-timer-tick-time-series.md)에 **측정 전에 고정**돼
있습니다. 남은 것은 사용자 gameplay 측정뿐입니다.

**스모크가 보여 준 것(pumpit1 attract, 44.5초):** 전달률이 구간마다 **25.0% → 98.6% →
48.1%** 로 요동하고 누적 평균은 43.1%입니다. **누적 평균은 어느 구간도 설명하지
않으므로, 이 축을 종료 요약 한 줄로 판단해서는 안 됩니다.**

**부수 확인 — 틱 주파수를 상수로 가정하면 안 됩니다.** 부팅 첫 1초의 `ticks_due`는
약 **18.2 Hz**(DOS 기본 BIOS 틱)이고 이후 **240 Hz**로 전이합니다.

**정정 — Tasks 421~423의 "후보 A~E 전부 기각"은 과했습니다.** 근거가 진행률·역행·정지
관측인데, 후보 A(장치 버퍼)와 E(pregap/논리 LBA)는 **일정한 오프셋**이라 그 측정으로는
보이지 않습니다. 증상이 "점프"이므로 우선순위는 낮지만 기각 상태는 아닙니다.

**정정 — `0x030F2786`은 정지 지점이 아닙니다.** 그 바이트는 `fb c3` = **`STI; RET`**
이며, 틱 대기 중 **가장 자주 트랩되는 특권 명령**일 뿐입니다. Task 420과 이 세션 초반이
이를 결정적 정지 주소로 읽은 것을 정정합니다. 또한 **single-step hotspot census로는 이
루프가 보이지 않습니다**(캐시 실행). 루프를 찾은 것은 `REPIU_GUEST_POSITION_CENSUS`입니다.
