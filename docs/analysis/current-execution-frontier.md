...
| 1'' | **무진행 감시 수정** — 진행 판정에 HLE 활동(MSCDEX·DOS)을 포함시키거나 틱 대기를 진행으로 인정해야 합니다. **선행 과제**: 지금은 정상 실행을 죽여 **프레임 기반 측정이 전부 왜곡**됩니다. 우회는 `REPIU_EXECUTION_TIMEOUT_MS=0` + 하니스 시간 제한 | 다른 모든 측정의 전제 | (측정 자체) |
