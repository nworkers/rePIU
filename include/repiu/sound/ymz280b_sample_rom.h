...
inline constexpr const char* kPiu10SampleRomEntry = "piu10.u9";

// Expected CRC32 of `piu10.u9` from the MAME PIU10 board ROM definition. Used only
// to report whether the loaded set matches the driver, never to reject a load.
inline constexpr std::uint32_t kPiu10SampleRomCrc32 = 0x9C436CFAU;
...
// Builds the 4 MiB sample address space from a PIU10 ROM-set ZIP by filling it with
