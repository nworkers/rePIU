...
Win32 native execution can use a fail-closed function return fast path implemented in `src/platform/win32/native_fast_path.*`. A region activates only after runtime offset and relocation-aware opcode signatures match. It clears Trap Flag while executing the original guest function and uses an x86 hardware execution breakpoint at the validated guest return address to reenter VEH. Any intermediate exception cancels the fast path and restores the prior debug registers and single-step state.
