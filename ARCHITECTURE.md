...
* `ImageMapper`: 원본 보호 모드 코드가 기대하는 메모리 이미지 구성. 현재 단계에서는 내부 relocation을 이미지 버퍼에 적용하는 dry-run까지 제공한다.
...
* `ImageMapper`: memory image expected by the original protected-mode code. The current step provides dry-run mapping with internal relocations applied to image buffers.
