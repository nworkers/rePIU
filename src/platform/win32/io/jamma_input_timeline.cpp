...
void Win32JammaInputTimeline::Reset(
...
void Win32JammaInputTimeline::RecordKeyEdge(
    std::uint64_t timestamp_nanoseconds,
    JammaInputKey key,
    bool pressed)
{
    ...
    const std::uint16_t next = pressed
...
bool Win32JammaInputTimeline::BeginTimerInterrupt(
    std::uint32_t pre_interrupt_esp,
    std::uint32_t interrupt_frame_esp)
{
    ...
    replay_frames_[replay_frame_depth_++] = {
        replay_timestamp, interrupt_frame_esp};
    ++replay_begin_count_;
    PruneHistoryLocked();
    return true;
}
...
void Win32JammaInputTimeline::PruneHistoryLocked()
{
    ...
    for (std::uint32_t index = 0; index < replay_frame_depth_; ++index)
    {
        const std::uint64_t timestamp =
            replay_frames_[index].timestamp_nanoseconds;
...
std::uint16_t Win32JammaInputTimeline::StateAtLocked(
    std::uint64_t timestamp_nanoseconds)
{
...
std::uint16_t CaptureCurrentJammaPressedMask()
{
    ...
    constexpr Mapping mappings[] = {
        {'Q', JammaInputKey::kP1Up},
        {'E', JammaInputKey::kP1Down},
        {'S', JammaInputKey::kP1Center},
        {'Z', JammaInputKey::kP1Left},
        {'C', JammaInputKey::kP1Right},
        {VK_F5, JammaInputKey::kCoin1},
        {VK_F1, JammaInputKey::kTest},
        {VK_F2, JammaInputKey::kService},
        {VK_F3, JammaInputKey::kClear},
        {VK_HOME, JammaInputKey::kP2Up},
        {VK_NUMPAD7, JammaInputKey::kP2Up},
        {VK_PRIOR, JammaInputKey::kP2Down},
        {VK_NUMPAD9, JammaInputKey::kP2Down},
        {VK_CLEAR, JammaInputKey::kP2Center},
        {VK_NUMPAD5, JammaInputKey::kP2Center},
        {VK_END, JammaInputKey::kP2Left},
        {VK_NUMPAD1, JammaInputKey::kP2Left},
        {VK_NEXT, JammaInputKey::kP2Right},
        {VK_NUMPAD3, JammaInputKey::kP2Right},
    };
    std::uint16_t pressed_mask = 0U;
    for (const Mapping& mapping : mappings)
    {
        if ((GetAsyncKeyState(mapping.virtual_key) & 0x8000) != 0)
        {
            pressed_mask = static_cast<std::uint16_t>(
                pressed_mask | JammaInputKeyMask(mapping.key));
        }
    }
    return pressed_mask;
}
